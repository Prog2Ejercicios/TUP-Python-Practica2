...
def suma_cubo_pares_sum_list(numeros: Iterable[int]) -> int:
    ...
    return sum([i ** 3 for i in numeros if not (i%2) != 0 ])
...
numeros_al_cubo_pares =  list(filter(lambda i: i%2 == 0, numeros_al_cubo))
...
suma_numeros_al_cubo_pares = reduce(lambda i, a: i + a, numeros_al_cubo_pares)
